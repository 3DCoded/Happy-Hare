...
```
  ...
  | `MMU_TEST_CONFIG` | Dump / Change essential load/unload config options at runtime | Many. Best to run MMU_TEST_CONFIG without options to report all parameters than can be specified <br>(Note: this only modifies Happy Hare klipper parameters; Happy Hare macro variables can be updated in a similar way using `SET_GCODE_VARIABLE`. See [Macro Variable Tip](Configuring-mmu_macro_vars.cfg#time-saving-tip)) |
