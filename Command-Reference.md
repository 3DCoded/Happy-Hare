...
<br>
  ...
  #### With "Linear_Selector" (ERCF, Tradrack, etc)
  | Command | Description | Parameters |
  | ------- | ----------- | ---------- |
  | `MMU_SERVO` | Set the servo to specified postion or a sepcific angle for testing. Will also report position when run without parameters | `POS=[up\|down\|move]` Move servo to predetermined position. <br>`ANGLE=..` Move servo to specified angle. <br>`SAVE=1` Specifed with the POS= parameter will cause Happy Hare to store the current servo angle for the specified position. This is written to `mmu_vars.cfg` <br>`RESET=1` This will reset the saved positions back to the defaults specified in `mmu_parameters.cfg`|

  #### With "Rotatary_Selector" (3DChameleon) or "Servo_Selector" (PicoMMU)
  | Command | Description | Parameters |
  | ------- | ----------- | ---------- |
  | `MMU_RELEASE` | Release the filament in the selected gate | |
  | `MMU_GRIP` | Grip the filament in the selected gate | |
