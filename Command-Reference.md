...
```
  ...
  | `MMU_CALIBRATE_SELECTOR` | **For "Rotary_Selector" (3DChameleon):** Calibration of the selector gate positions. Usage: MMU_CALIBRATE_SELECTOR [GATE=x] [SAVE=0\|1] [SINGLE=0\|1] [QUICK=[0\|1] | Store the current selector angle for the specified gate position. This is written to `mmu_vars.cfg` <br>`GATE=[0..n]` The individual gate position to calibrate. <br>`QUICK=[0\|`] If set to 1 and an endstop is fitted, then all gate gate positions will be automatically set based on the MMU geometry (they can still be overridden/tuned with the "SINGLE" parameter later). <br>`SINGLE=[0\|1]` If set to 1 then the individual gate position will be saved. This is useful to fine tune each gate separately |
  | `MMU_CALIBRATE_SELECTOR` | **For "Servo_Selector" (PicoMMU):** Calibration of the selector gate positions. Usage: MMU_CALIBRATE_SELECTOR [GATE=x] [BYPASS=0|1] ANGLE=x] [SAVE=0|1] [SINGLE=0|1] | `ANGLE=..` Move to the specified angle. <br>`SAVE=1` Specifed with the POS= parameter will cause Happy Hare to store the current selector angle for the specified position. This is written to `mmu_vars.cfg` <br>`GATE=[0..n]` The individual gate position to calibrate. <br>`BYPASS=[0\|1]` Calibrate the bypass position. <br>`SPACING=,,` Specifies the servo angle delta between each gate selection. By default setting any gate will cause other gates to be automatically extrapolated. <br>`SINGLE=[0\|1]` If set to 1 then the individual gate position will be saved. This is useful to fine tune each gate separately and turns off extrapolation |
