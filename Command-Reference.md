...
<br>
  ...
  | `MMU_SERVO` | **<u>For "Linear_Selector" (ERCF, Tradrack)</u>:** <br>Set the servo to specified postion or a sepcific angle for testing. Will also report position when run without parameters | `POS=[up\|down\|move]` Move servo to predetermined position. <br>`ANGLE=..` Move servo to specified angle. <br>`SAVE=1` Specifed with the POS= parameter will cause Happy Hare to store the current servo angle for the specified position. This is written to `mmu_vars.cfg` <br>`RESET=1` This will reset the saved positions back to the defaults specified in `mmu_parameters.cfg`|
...
```
  ...
  | `MMU_CALIBRATE_SELECTOR` | **<u>For "Rotary_Selector" (3DChameleon)</u>:** <br>Calibration of the selector gate positions. Usage: MMU_CALIBRATE_SELECTOR [GATE=x] [SAVE=0\|1] [SINGLE=0\|1] [QUICK=[0\|1] | Store the current selector angle for the specified gate position. This is written to `mmu_vars.cfg` <br>`GATE=[0..n]` The individual gate position to calibrate. <br>`QUICK=[0\|`] If set to 1 and an endstop is fitted, then all gate gate positions will be automatically set based on the MMU geometry (they can still be overridden/tuned with the "SINGLE" parameter later). <br>`SINGLE=[0\|1]` If set to 1 then the individual gate position will be saved. This is useful to fine tune each gate separately |
