...
## ![#f03c15](resources/f03c15.png) ![#c5f015](resources/c5f015.png) ![#1589F0](resources/1589F0.png) Supported Sensors
  ...
  | Sync-Feedback Sensors<br>`filament_compression`<br>`filament_tension` |
These one or two optional sensors (Happy Hare can support either or both) are fitted into a device often called a "buffer" and detect filament compression (MMU pushing filament faster than the extruder is pulling) or filament tension (extruder is pulling faster than MMU is pushing).<br>&nbsp;<br>**Primary Functions:**<br>1. Stepper snychronisation - Allows synchronization of the gear stepper on the MMU and the extruder on the printer. This allows the MMU to contribute to feeding filament without interfering with the extruder operation and causing print artifacts.<br>2. Extruder endstop - The **filament_compression** sensor can be used to detect the filament hitting the extruder entry and thus be used as a accurate homing point close to the extruder.<br>&nbsp;<br>3. Bowden calibration - The **filament_compression** sensor can also be used to easily automate the calibration of bowden length by detecting the entrance to the extruder.<br>&nbsp;<br>**Secondary Functions:**<br>4. Other theoretical uses like runout detection are possible but depend on the sensor unit design specifics (e.g. is centrally sprung). Currently Happy Hare does not support other uses. |
